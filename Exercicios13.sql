...
SELECT ca.marca, ca.modelo, SUM(vi.distanciaPercorrida) as totalDistancia
...
SELECT em.nome FROM empresa em LEFT OUTER JOIN carro ca
ON em.id = ca.idEmpresa
WHERE em.id NOT IN(
		SELECT ca.idEmpresa FROM carro ca
		)

--Consultar Marca e modelos dos carros que não fizeram viagens

SELECT ca.marca, ca.modelo FROM carro ca LEFT OUTER JOIN viagem vi
ON ca.id = vi.idCarro
WHERE ca.id NOT IN(
		SELECT vi.idCarro FROM viagem vi
		)
...
SELECT em.nome, ca.marca, ca.modelo, COUNT(vi.idCarro) as qtdViagem FROM carro ca, viagem vi, empresa em
WHERE ca.id = vi.idCarro
		AND ca.idEmpresa = em.id
GROUP BY em.nome, ca.marca, ca.modelo
ORDER BY em.nome ASC, qtdViagem DESC
...
SELECT em.nome, ca.marca, ca.modelo, vi.distanciaPercorrida,
CASE WHEN (vi.distanciaPercorrida < 1000)
THEN (vi.distanciaPercorrida * 10)
ELSE (vi.distanciaPercorrida * 15)
END AS valorGanho
FROM empresa em, carro ca, viagem vi
WHERE em.id = ca.idEmpresa
		AND ca.id = vi.idCarro
